...
// Minimal ABI (your contract needs these)
const NFT_ABI = [
  {
...
export default function DukeNukemLandingPage() {
  ...
  const [metaName, setMetaName] = useState("Duke PFP #1");
  const [metaDesc, setMetaDesc] = useState("Born to kick ass and chew bubblegum.");
  ...
  // Prepare “Duke style” into offscreen canvas; keep hidden pre-mint
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900/50 p-5">
              ...
              <div className="mt-6 text-xs text-neutral-400">
                Tip: Lighthouse has encryption/token-gating add-ons if you want to evolve this later.
              </div>
            </div>
          </motion.div>
        )}
