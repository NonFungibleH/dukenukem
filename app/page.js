...
const CONTRACT_ADDRESS = "0x2162924dae5b87b593fe62205f045a26edf27548"; // Verify this address on Sepolia
const CONTRACT_ABI = [
  {
    "inputs": [
      {
...
export default function DukeNukemLandingPage() {
  ...
  const canMint = isConnected && !!dukeUrl && !hasMinted && nftName && nftDescription && walletClient;

  // Initialize walletClient and ensure MetaMask is on Sepolia
  useEffect(() => {
    async function initializeWalletClient() {
      if (typeof window === "undefined" || !window.ethereum) {
        setError("MetaMask not detected. Please install MetaMask or use a compatible wallet.");
      ...
      try {
        ...
        console.log("walletClient initialized:", !!client);
      } catch (err) {
  ...
  // Check mint status and fetch mint fee
  useEffect(() => {
    async function checkMintStatus() {
      ...
      try {
        ...
        console.log("Checking hasMinted for address:", address);
        ...
        console.log("hasMinted result:", result);
        setHasMinted(result);
        ...
        setError(`Failed to check mint status: ${err.message}. Verify the contract address and network.`);
      }
    }
    checkMintStatus();
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const chainId = await window.ethereum.request({ method: "eth_chainId" });
      console.log("Minting chainId:", chainId);
      if (chainId !== `0x${sepolia.id.toString(16)}`) {
        await window.ethereum.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId: `0x${sepolia.id.toString(16)}` }],
      ...
      console.log("Simulated mintNFT request:", request);
      ...
      if (receipt.status === "success") {
        ...
        const nftMintedEvent = receipt.logs.find(
          (log) =>
            log.topics[0] ===
            "0x3e71b3536c6632c7b6e0349b5b3f1e0d66e12dc2f65c5a23103f0b43101c27df"
        );
        if (nftMintedEvent) {
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20">
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-neutral-800 bg-neutral-900/50 p-5">
            ...
            <p className="mt-1 text-xs text-neutral-400">Sepolia Testnet address</p>
          ...
          >
            <div className="rounded-2xl border border-amber-500/30 bg-neutral-900/60 p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <a
                  href={`https://sepolia.etherscan.io/token/${CONTRACT_ADDRESS}?a=${mintedTokenId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-xl bg-neutral-800 px-4 py-2 text-sm hover:bg-neutral-700"
                >
